...
public class JobListDecimal {
     ...
     * Runs O( empty ? 1 : n )
     */
    public JobListDecimal(JobNode[] jobs, boolean empty) throws Exception {
        this.jobs = jobs;
     ...
     * Runs O(n)
     */
    public JobListDecimal split(int x) throws Exception {
        int lengthL = 0;
        int totalPL = 0;
        ...
        while (current != null) {
            if (current.index >= x) {
                ...
                JobListDecimal right = new JobListDecimal(jobs, true);
                right.start = current;
                right.end = end;
        ...
        return new JobListDecimal(jobs, true); // O(1)
    }
    ...
    static class JobNode {
        ...
        public JobNode(int index, int p, float d) {
            this.index = index;
            this.p = p;
            this.d = d;
        }
